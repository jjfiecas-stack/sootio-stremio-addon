  ...
  if (mediaTitleWords.length > 0) {
    ...
    if (resultTitleWords.length >= mediaTitleWords.length) {
      ...
      if (startsWithTitle) {
        ...
        // Fallback: Try matching without leading articles (the, a, an)
        // Handles cases where one title has "The Bibi Files" and the other has "Bibi Files"
        if (!titleMatches) {
          const articles = ['the', 'a', 'an'];
          const mediaWithoutArticle = articles.includes(mediaTitleWords[0]) ? mediaTitleWords.slice(1) : mediaTitleWords;
          const resultWithoutArticle = articles.includes(cleanedWords[0]) ? cleanedWords.slice(1) : cleanedWords;

          if (mediaWithoutArticle.length > 0 && resultWithoutArticle.length >= mediaWithoutArticle.length) {
            const mediaPatternNoArticle = mediaWithoutArticle.join(' ');
            const resultNoArticle = resultWithoutArticle.join(' ');
            if (resultNoArticle.startsWith(mediaPatternNoArticle + ' ') ||
                resultNoArticle === mediaPatternNoArticle) {
              console.log(`[UHDMovies] Matched after stripping article: "${mediaPatternNoArticle}"`);
              titleMatches = true;
            }
          }
        }
